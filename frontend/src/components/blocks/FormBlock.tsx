        ...
        >
          {(block.fields ?? []).map((field) => (
            <input
              ...
              className="h-9 w-[288px] rounded-md border border-white bg-transparent px-[18px] text-[15.75px] leading-[22.5px] text-white placeholder:text-white outline-none focus:border-sky md:h-11"
            />
          ))}
          <button
            type="submit"
            disabled={state === "submitting"}
            className={`mo-btn h-9 shrink-0 rounded-md bg-mint px-9 font-[family-name:var(--font-franklin)] text-[15.75px] font-bold leading-[22.5px] text-black transition disabled:opacity-60 md:h-11 ${submitted ? "mo-confirm" : ""}`}
          >
            {state === "submitting"
              ? "Sending…"
              : submitted
